...
CC=icpc -openmp
OPTIMIZATION=-O3
CFLAGS=-ansi -std=c++11  -pedantic  $(OPTIMIZATION) #-vec-report=2
#CFLAGS=-ansi -std=c++11 -Iincludes $(OPTIMIZATION)
...
zip: clean
ifdef TEAM_ID
	zip $(strip $(TEAM_ID)).zip -9r Makefile Readme src/ obj/ includes/
else
